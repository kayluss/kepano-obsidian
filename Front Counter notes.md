---
created: 2025-12-27
tags:
  - note
  - journal
categories:
  - "[[Jobs]]"
---
FC means let the runner take care of orders, stock soda area clean lobby, clean trays. 

## Order Taking
	Ask to round up
	Pennies- round to the nearest nickel, Let them know
	Eat in requires tag number, as order brought out to customer.
	
## Parked orders
	PLace receipts left to right on fry station in order of where they are parked.
## Drinks
	Sizes Soda - Lg 32, Md 22, Sm 16 
	
	 Iced coffee
		L = 6 pumps of syrup
		M = 5 Pumps of syrup
		S = 4 pumps of syrup
		Process
			To make an iced coffee at McDonald's, employees 
			combine ice, a liquid sugar or flavored syrup, light cream, and their brewed premium roast coffee in a cup==. 
	Ingredients
	- **Ice:** Fills the cup up to a specific line.
	- **Syrup:** A specific number of pumps of flavored syrup (vanilla, sugar-free vanilla, or caramel) or unflavored liquid sugar, depending on the size of the drink.
	- **Cream:** A set amount of light cream is added, often automated by the machine. In Canada, you can also opt for 2% milk.
	- **Brewed Coffee:** McDonald's uses a strong, proprietary blend of 100% Arabica bean coffee, brewed fresh. 
	Steps for Preparation
	1. **Add ice:** The cup is filled with ice to the designated fill line.
	2. **Add flavor and cream:** The correct number of pumps of desired syrup or liquid sugar and the required amount of cream are dispensed into the cup.
	3. **Pour coffee:** Freshly brewed, strong coffee is added to the mixture.
	4. **Stir:** The drink is stirred (around 12 times for a medium) to combine all the ingredients thoroughly.

## Oatmeal
	**Here's the process:**
		1. **Order:** Ask for the Fruit & Maple Oatmeal.
		2. **Components:** You'll receive a bowl, an oatmeal packet, a packet of fruit (diced apples, raisins, cranberries), and usually a small cup of light cream.
		3. **Add Hot Water:** Fill the bowl with hot water from the dispenser, often to a fill line.
		4. **Add Cream (Optional):** Pour in a splash of the light cream for a richer flavor, or skip it if you want it plain.
		5. **Mix Packet:** Empty the oatmeal packet and brown sugar into the hot water and stir.
		6. **Add Fruit:** Top with the fruit mix, spreading the cranberries and apples evenly.
		7. **Serve:** Place the lid on and enjoy!.
## Register 
	Clear Choice button to fix sauces/drinks errors in ringing up
	app codes - My Customer button
	Reciept Code - item -> promo button
## Runner
	place order list on bag announce number or name if there is one	 
	